...
app.listen(PORT, async () => {
    ...
    if (process.env.USE_NGROK === 'true') {
        try {
            ...
            logger.info(`Dynamic M-Pesa Callback URL: ${process.env.MPESA_CALLBACK_URL}`);
        } catch (err) {
            logger.error('Ngrok Error: %o', err);
            logger.info(`Falling back to static Callback URL: ${process.env.MPESA_CALLBACK_URL}`);
        }
    } else {
        logger.info('Ngrok is disabled. Using static Callback URL: %s', process.env.MPESA_CALLBACK_URL);
    }
});
